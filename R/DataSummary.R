...
#' @usage DataSummary(data,missing=list(NA),wt=NULL,entropy_method="emp",sparkline=FALSE)
...
#' @param entropy_method The name of the entropy estimator. The `infotheo` package implements four estimators : "emp", "mm", "shrink", "sg" (default:"emp").
...
DataSummary <- function(data,missing=list(NA),wt=NULL,entropy_method="emp",sparkline=FALSE){
  UseMethod("DataSummary",data)
}

#' @export
#' @rdname DataSummary
DataSummary.data.frame <- function(data,missing=list(NA),wt=NULL,entropy_method="emp",sparkline=FALSE){
  setDT(data)
  DataSummary.data.table(data,missing,wt,entropy_method,sparkline)
}

#' @export
#' @rdname DataSummary
#' 
DataSummary.data.table <- function(data,missing=list(NA),wt=NULL,entropy_method="emp",sparkline=FALSE){
  ...
  dsEntrophy <- data[,sapply(data,function(x) infotheo::entropy(infotheo::discretize(x), method = entropy_method))]
